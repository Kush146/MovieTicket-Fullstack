...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between 
      px-5 sm:px-10 md:px-20 lg:px-40 py-4 md:py-5 backdrop-blur bg-black/20'>
      ...
      <Link to='/' className='flex-1 flex items-center min-w-0'>
        <img
          src={assets.logo}
          alt="KukiShow Logo"
          className="h-8 sm:h-10 md:h-12 w-auto max-w-[70vw] object-contain"
        />
      </Link>
      ...
      >
        ...
        {/* Added Search Icon inside nav group with left margin */}
        <SearchIcon className='hidden md:inline-block w-5 h-5 ml-2 cursor-pointer hover:opacity-80 transition' />
      ...
      {/* Search + Login */}
      <div className='flex items-center gap-6 sm:gap-8 pr-2 sm:pr-4 md:pr-6 lg:pr-8 ml-3 sm:ml-5'>
        {/* Removed extra SearchIcon from here to avoid duplicate */}
        {!user ? (
          <button
            onClick={openSignIn}
